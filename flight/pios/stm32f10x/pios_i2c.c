...
#define I2C_SLV_ERROR_SIZE 4
uint8_t error_msg[I2C_SLV_ERROR_SIZE] = {
	0xDE, 
	0xAD,
	0xBE,
	0xEF,
...
int32_t i2c_slv_error_idx = 0;

void PIOS_I2C_Slv_Send_Data(struct pios_i2c_adapter *i2c_adapter) {
	if(i2c_adapter->i2c_slv_tx_len == 0) {
		i2c_adapter->i2c_slv_tx_idx = 0;
		//Could also send a certain error code, ie) 0xDEADBEEF
		I2C_SendData(i2c_adapter->cfg->regs, 
				error_msg[i2c_slv_error_idx++]);
		i2c_slv_error_idx %= I2C_SLV_ERROR_SIZE;
	} else {
		I2C_SendData(i2c_adapter->cfg->regs, 
			i2c_adapter->i2c_slv_tx_buf[
			i2c_adapter->i2c_slv_tx_idx++]);//++new_irq_data);
		/*
		//Can change to setting len and idx to 0 when done
		...
		if(i2c_adapter->i2c_slv_tx_idx >= 
				i2c_adapter->i2c_slv_tx_len) {
			i2c_adapter->i2c_slv_tx_idx = 0;
			i2c_adapter->i2c_slv_tx_len = 0; //done
		}
	}
}
...
void PIOS_I2C_EV_IRQ_Handler(uint32_t i2c_id) {
	...
	switch(event) {
		...
		case I2C_EVENT_SLAVE_TRANSMITTER_ADDRESS_MATCHED: //EV1
			I2C_clear_ADDR(i2c_adapter->cfg->regs);
			//Send first byte
			PIOS_I2C_Slv_Send_Data(i2c_adapter);
			//i2c_adapter->i2c_slv_tx_idx = 0;
